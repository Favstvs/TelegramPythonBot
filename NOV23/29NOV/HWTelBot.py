...
def protecchusbando(update: Update, context: CallbackContext):
    ...
    if data[0]:
        ...
        if protecc:
            ...
            # Verifico se corrisponde
            if HfindWholeWord(protecc, Husbando):
...
def Hharem(update: Update, context: CallbackContext):
    ...
    if data:
        ...
        # Se esiste una sua scheda messaggi la aggiorno
        # Altrimenti la creo
        HCheckMessages(ID_Supergruppo, ID_User, Mess_ID_List.message_id)
        # Aggiorno le info dell'utente (Username)
        CheckUser(ID_User, Username)
    else:
        ...
        try:
            ...
            if 5 <= NewTime <= 20:
                ...
                mycursor.execute("""UPDATE Hmanagement
                                    SET Time_reset = %s
                                    WHERE ID_Supergruppo = %s
                                    """, (NewTime + 1, ID_Supergruppo,))
                
                update.message.reply_text(
                    "Tempo cambiato!\nDa ora waifu e husbandi appariranno dopo " + str(NewTime) + " messaggi")
            else:
...
def UpdatePacks(update: Update, context: CallbackContext):
 ...
 ##########################################################
    ...
    #CallBackRequest = str(update.callback_query.data)
    Time_Mess_Packs = update.effective_message.date
    ...
    if data[0] == None:
        ...
        if diff_in_hours > 8: #Ogni 8 ore, altrimenti RIFIUTA
           ...
           update.effective_message.reply_text("Avete già usato il pack, dovete aspettare 8 ore")
...
def CheckMessages(ID_Supergruppo, ID_User, Mess_ID_List):
    ...
    if data:
        ...
        # Setto i dati della nuova relazione
        mycursor.execute("INSERT INTO Wharem(ID_Supergruppo, ID_User, Mess_ID_List) "
                         "VALUES(%s, %s, %s)",
                         (ID_Supergruppo, ID_User, Mess_ID_List))


def HCheckMessages(ID_Supergruppo, ID_User, Mess_ID_List):
    # Inserisco il Mess_ID del suo trade nei dati utente
    mycursor.execute("SELECT ID_User "
                     "FROM Hharem "
                     "WHERE ID_User=%s AND ID_Supergruppo = %s",
    ...
    if data:
        ...
        # Setto i dati della nuova relazione
        mycursor.execute("INSERT INTO Hharem(ID_Supergruppo, ID_User, Mess_ID_List) "
                         "VALUES(%s, %s, %s)",
                         (ID_Supergruppo, ID_User, Mess_ID_List))
...
def HUpdateGroup(ID_Supergruppo, context: CallbackContext):
    # Aggiorno il numero di messaggi prima del prossimo spawn
    mycursor.execute("""SELECT Time_mess, Started
                        FROM Hmanagement
                        WHERE ID_Supergruppo = %s""",
...
def HfindWholeWord(protecc, husbando):
    for protecc_word in protecc.split(" "):
        for husbando_word in husbando.split(" "):
            if protecc_word == husbando_word:
                return True
    return False
...
def Welcomechat(update: Update, context: CallbackContext):
    ...
    if update.message.new_chat_members[0].id == context.bot.id:
        ...
        update.message.reply_text(text="<b>Ciao chat!</b> Grazie per avermi aggiunto! "
        		 		"Da ora un sacco di waifu e hsubandi appariranno in questo gruppo "
...
def NewGroup(ID_Supergruppo, Supergruppo_nome):
    ...
    if not data:
        ...
        mycursor.execute(
            "INSERT INTO Wmanagement(ID_Supergruppo, Time_mess, Time_reset, Started) VALUES (%s,%s,%s,%s)",
            (ID_Supergruppo, 1, 100, 0))
        mycursor.execute(
            "INSERT INTO Hmanagement(ID_Supergruppo, Time_mess, Time_reset, Started) VALUES (%s,%s,%s,%s)",
            (ID_Supergruppo, 1, 100, 0))

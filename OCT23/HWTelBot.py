...
def Wharem(update: Update, context: CallbackContext):
    ...
    # Cerco le relation
    mycursor.execute("""SELECT relation.Place, waifu.Nome_Waifu, relation.NP, waifu.ID_Waifu
                        FROM waifu, relation
                        WHERE relation.ID_User= %s AND
                     relation.ID_Supergruppo = %s AND
                     relation.ID_Waifu= waifu.ID_Waifu
                        ORDER BY Place asc 
                        LIMIT 21""",
                     (ID_User, ID_Supergruppo,))
...
def PageSelection(update: Update, context: CallbackContext):
    ...
    if VerifyListIdentity(Mess_ID, ID_Supergruppo, ID_User):
        ...
        mycursor.execute("""SELECT relation.Place, waifu.Nome_Waifu, relation.NP 
                                    FROM waifu, relation
                                    WHERE relation.ID_User= %s AND
                                 relation.ID_Supergruppo = %s AND
                                 relation.ID_Waifu = waifu.ID_Waifu AND
        ...
        if data:
            ...
            mycursor.execute("""SELECT COUNT(*) 
                                    FROM ( 
                                        SELECT relation.ID_Waifu
        	                            FROM waifu, relation
        	                            WHERE relation.ID_User= %s AND
        	                         relation.ID_Supergruppo = %s AND
        	                         relation.Place > %s + 20 AND
        	                         relation.ID_Waifu = waifu.ID_Waifu
        	                            LIMIT 20
                                        ) AS count""",
                             (ID_User, ID_Supergruppo, New_Page,))
...
def main():
    ...
    #dp.add_handler(CommandHandler("hgift", gift, Filters.chat_type.supergroup & Filters.update.message))
    ...
    #dp.add_handler(CommandHandler("hharem", harem, Filters.chat_type.supergroup & Filters.update.message))
    #dp.add_handler(CallbackQueryHandler(PageSelection))
